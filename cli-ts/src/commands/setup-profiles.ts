...
import type { ProfilesUpdatePayload } from "../types.js";
...
async function setProfiles(baseUrl: string, settings: ProfilesUpdatePayload): Promise<any> {
...
export async function handler(argv: { targethost: string; file: string, [key: string]: any }) {
  ...
  try {
    ...
    const settings: ProfilesUpdatePayload = JSON.parse(fileContent);
