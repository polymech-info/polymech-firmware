...
export const builder: CommandBuilder<Options, Options> = (yargs) =>
  yargs
    ...
    .option('query', {
        ...
        coerce: (arg) => queryPresets[arg as keyof typeof queryPresets] || arg,
    })
    .option('verbose', {
        alias: 'v',
        type: 'boolean',
      ...
      if (messageData) {
        ...
        if (query) {
            try {
                ...
                if (result.length > 0) {
                    if (verbose) console.log(JSON.stringify(result[0], null, 2));
                }
            } catch (e) {
                // Ignore errors for messages that don't match the query structure
            }
        } else {
             if (verbose) console.log(JSON.stringify(messageData, null, 2));
        }
      }
    });
