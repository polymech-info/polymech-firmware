...
type ComponentMap = Map<string, string>; // Map<ClassName, AbsoluteHeaderPath>

// 1. Parse a C++ file and extract all defines (commented and uncommented)
function parseDefines(fileContent: string): Map<string, { value: string | null; isComment: boolean }> {
    const defines = new Map<string, { value: string | null; isComment: boolean }>();
...
export async function buildFeatureCatalog(projectRoot: string): Promise<Map<string, string>> {
    ...
    function findIfdefs(node: Parser.SyntaxNode) {
...
export async function buildClassNameToFeatureMap(featureCatalog: Map<string, string>, includeDirs: string[]): Promise<Map<string, string>> {
    const map = new Map<string, string>();

    // --- AST DUMP LOGIC ---
    const astDir = path.join(process.cwd(), 'ast');
    await fs.mkdir(astDir, { recursive: true });
    function serializeNode(node: Parser.SyntaxNode): any {
...
export async function analyzeComponentDependencies(headerPath: string, includeDirs: string[], classNameToFeatureMap: Map<string, string>): Promise<string[]> {
    ...
    for (const declaration of constructorDeclarations) {
        ...
        if (functionName !== className) continue; // It's a destructor, skip it
...
function findClassName(rootNode: Parser.SyntaxNode): string | null {
...
export async function generateFeatureTree(projectRoot: string): Promise<Record<string, FeatureDetails>> {
    ...
    const finalTree: Record<string, FeatureDetails> = {};
...
export async function findComponents(includeDirs: string[]): Promise<ComponentMap> {
    const componentMap: ComponentMap = new Map();
    const parser = new Parser();
    parser.setLanguage(Cpp);

    const childByType = (node: Parser.SyntaxNode, type: string) => {
        for (const child of node.children) {
    ...
    const childrenByType = (node: Parser.SyntaxNode, type: string) => {
...
async function getDependencies(
    ...
    const dependencies: ComponentNode[] = [];
    const constructorNode = classNode.descendantsOfType('declaration').find(decl =>
        decl.childForFieldName('declarator')?.childForFieldName('declarator')?.text === className
    );
