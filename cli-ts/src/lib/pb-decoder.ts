...
export interface DecodedNV {
  address: number;
  timestamp: number;
  value: number | boolean | Buffer | number[];
}
 ...
 */
export function decodeNetworkValue(buffer: Buffer): DecodedNV {
  const reader = Reader.create(buffer);
  const result: Partial<DecodedNV> = {};
  let value: any = null;
  ...
  while (reader.pos < reader.len) {
    ...
    switch (fieldNumber) {
      ...
      case 2: // timestamp
        ...
        result.timestamp = reader.uint64() as unknown as number;
        break;
      case 3: // sint_value
        value = reader.sint64() as unknown as number;
        break;
      case 4: // bool_value
        value = reader.bool();
        break;
