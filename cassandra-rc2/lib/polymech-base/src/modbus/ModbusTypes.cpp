...
void RegisterState::printState(ModbusRTU &manager, uint8_t slaveId)
{
  ...
  {
  case E_FN_CODE::FN_READ_INPUT_REGISTER:
    Log.infoln("Input Register %d: %d (Priority: %d, %s)",
               address, value, priority,
               manager.isRegisterSynchronized(slaveId, address) ? "Synchronized" : "Not synchronized");
    break;

  case E_FN_CODE::FN_READ_HOLD_REGISTER:
    Log.infoln("Holding Register %d: %d (Priority: %d, %s)",
               address, value, priority,
               manager.isRegisterSynchronized(slaveId, address) ? "Synchronized" : "Not synchronized");
    break;

  case E_FN_CODE::FN_READ_COIL:
    Log.infoln("Coil %d: %s (Priority: %d, %s)",
               address, getBoolValue() ? "ON" : "OFF", priority,
               manager.isCoilSynchronized(slaveId, address) ? "Synchronized" : "Not synchronized");
    break;

  case E_FN_CODE::FN_READ_DISCR_INPUT:
    Log.infoln("Discrete Input %d: %s (Priority: %d, %s)",
               address, getBoolValue() ? "ON" : "OFF", priority,
               manager.isCoilSynchronized(slaveId, address) ? "Synchronized" : "Not synchronized");
    break;

  default:
    Log.infoln("Register Address %d: Value %d (Type: %X, Priority: %d, %s)",
...
bool DuplicateOperationFilter::filter(ModbusOperation &op)
{
...
bool RateLimitFilter::filter(ModbusOperation &op)
{
...
bool PriorityFilter::filter(ModbusOperation &op)
{
  Manager *manager = Manager::getInstance();
  if (manager)
  {
    RTU_Base *device = manager->getDeviceById(op.slaveId);
    if (device && device->isHighPriority(op))
    {
      op.setHighPriority(true);
    }
  }
  return true; // Use existing logic if any, currently just return true
}
...
{
  // Do not queue a new read operation if:
  // 1. The device component is disabled.
  // 2. An operation for this device is already in progress ('RUNNING' state).
  //    This prevents queuing duplicate reads.
  // 3. The device is in a backoff period due to previous communication errors (e.g., timeout).
  if (!enabled() || state == RUNNING || !manager.isDeviceReady(this->slaveId))
  {
    return;
  }
...
void RTU_Base::write(ModbusRTU &manager)
{
  ...
  // LS_INFO(F("RTU_Base::write(slaveId: %d) - Iterating %d output registers."), slaveId, registerCount);
  bool writeQueued = false;
  for (int i = 0; i < registerCount; i++)
  {
    if (registers[i] != nullptr && registers[i]->dirty)
    {
      if (registers[i]->type == E_FN_CODE::FN_WRITE_HOLD_REGISTER ||
          registers[i]->type == E_FN_CODE::FN_WRITE_COIL)
      {
        // LS_INFO(F("RTU_Base::write(slaveId: %d) - Processing dirty register index %d: address 0x%04X, value %d"), slaveId, i, registers[i]->address, registers[i]->value);
...
void RTU_Base::printState()
{
  for (int i = 0; i < registerCount; i++)
  {
    ...
    {
      Log.infoln("Register %d: Type %d, Address %d, Value %d", i, registers[i]->type, registers[i]->address, registers[i]->value);
    }
  }
}

short RTU_Base::reset()
{
...
bool RTU_Base::setOutputRegisterValue(uint16_t address, uint16_t value, bool force)
{
  bool found = false;
  for (int i = 0; i < registerCount; i++)
  {
    if (registers[i] != nullptr && registers[i]->address == address)
    {
      ...
      {
        if (registers[i]->value != value || force)
        {
          registers[i]->value = value;
          registers[i]->dirty = true;
        }
        found = true;
        break;
      }
...
bool RTU_Base::onRegisterUpdate(uint16_t address, uint16_t newValue)
{
  bool updated = false;
  for (int i = 0; i < registerCount; ++i)
  {
