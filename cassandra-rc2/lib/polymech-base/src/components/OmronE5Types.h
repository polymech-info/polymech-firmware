#ifndef OMRON_E5_TYPES_H
#define OMRON_E5_TYPES_H

// Omron EJ5 Modbus Registers & Coils

// Correctly checks a bit in a 32-bit status word composed of a High and Low 16-bit register.
#define OR_E5_STATUS_BIT(H, L, B) ((B) < 16 ? (((L) & (1 << (B))) != 0) : (((H) & (1 << ((B) - 16))) != 0))

// DEPRECATED - This is confusing. Use the specific command enums and builder macro below.
...
enum OR_E5_STATUS_1
{
    ...
    OR_E5_S1_PotentiometerInputError = 7,
...
// The register address for sending operation commands.
#define OR_E5_OPERATION_COMMAND_REGISTER 0x0000

// Command Codes for the Operation Command Register.
// These are written to the high byte of the command word.
enum OR_E5_OPERATION_CODE {
    OP_CODE_COMMS_WRITE = 0x00,
    OP_CODE_RUN_STOP = 0x01,
    OP_CODE_MULTI_SP = 0x02,
    OP_CODE_AT_EXECUTE_CANCEL = 0x03,
    OP_CODE_WRITE_MODE = 0x04,
    OP_CODE_SAVE_RAM_DATA = 0x05,
    OP_CODE_SOFTWARE_RESET = 0x06,
    OP_CODE_MOVE_TO_SETUP_AREA_1 = 0x07,
    OP_CODE_MOVE_TO_PROTECT_LEVEL = 0x08,
    OP_CODE_AUTO_MANUAL_SWITCH = 0x09,
    OP_CODE_PARAMETER_INITIALIZATION = 0x0B,
    OP_CODE_ALARM_LATCH_CANCEL = 0x0C,
    OP_CODE_SP_MODE = 0x0D,
    OP_CODE_INVERT_DIRECT_REVERSE = 0x0E,
    OP_CODE_PROGRAM_START = 0x11
};

// Data for the Operation Command Register.
// These are written to the low byte of the command word.
enum OR_E5_OPERATION_DATA {
    // For OP_CODE_COMMS_WRITE
    COMMS_WRITE_DISABLED = 0x00,
    COMMS_WRITE_ENABLED = 0x01,

    // For OP_CODE_RUN_STOP
    RUN_CMD = 0x00,
    STOP_CMD = 0x01,

    // For OP_CODE_AT_EXECUTE_CANCEL
    AT_CANCEL_CMD = 0x00,
    AT_EXECUTE_100_CMD = 0x01,
    AT_EXECUTE_40_CMD = 0x02
};

// Helper macro to build the 16-bit command word for the Operation Command Register
#define BUILD_OMRON_OP_COMMAND(code, data) ((((uint16_t)code) << 8) | (uint8_t)data)


enum OR_E5_CMD_ADDRESS
{
    OR_E5_CMD_COM_WRITE = 0,    // DEPRECATED
    OR_E5_CMD_STOP_RUN = 0,     // DEPRECATED
    OR_E5_CMD_AT = 0x200
};

enum OR_E5_CMD
{
    // These are DEPRECATED and will be removed. Use BUILD_OMRON_OP_COMMAND instead.
    OR_E5_STOP = 1,
    OR_E5_RUN = 0,
