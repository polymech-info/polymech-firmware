...
SAKO_VFD::SAKO_VFD(Component* owner, uint8_t slaveId, millis_t readInterval)
    : VFD_Base(owner, slaveId, COMPONENT_KEY_SAKO_VFD, readInterval)
{
    componentId = id;
    ...
    _modbusBlocks[8] = INIT_MODBUS_BLOCK(E_SakoTcpOffset::STATE, E_FN_CODE::FN_READ_HOLD_REGISTER, MB_ACCESS_READ_ONLY, "SAKO: State", name.c_str());
    _modbusBlocks[9] = INIT_MODBUS_BLOCK(E_SakoTcpOffset::CMD_FREQ, E_FN_CODE::FN_WRITE_HOLD_REGISTER, MB_ACCESS_READ_WRITE, "SAKO: Set Freq Cmd", name.c_str());
    _modbusBlocks[10] = INIT_MODBUS_BLOCK(E_SakoTcpOffset::CMD_DIRECTION, E_FN_CODE::FN_WRITE_HOLD_REGISTER, MB_ACCESS_WRITE_ONLY, "SAKO: Direction Cmd", name.c_str());
    _modbusBlocks[11] = INIT_MODBUS_BLOCK(E_SakoTcpOffset::CMD_COMMAND, E_FN_CODE::FN_WRITE_HOLD_REGISTER, MB_ACCESS_READ_WRITE, "SAKO: Command", name.c_str());
    _modbusBlocks[12] = INIT_MODBUS_BLOCK(E_SakoTcpOffset::TARGET_REGISTER, E_FN_CODE::FN_WRITE_HOLD_REGISTER, MB_ACCESS_READ_WRITE, "SAKO: Target Reg", name.c_str());
    _modbusBlocks[13] = INIT_MODBUS_BLOCK(E_SakoTcpOffset::TARGET_VALUE, E_FN_CODE::FN_WRITE_HOLD_REGISTER, MB_ACCESS_READ_WRITE, "SAKO: Target Val", name.c_str());
    L_INFO("SAKO_VFD: Modbus Blocks: Start Address %d", tcpBaseAddr);
...
{
    L_INFO(F("SAKO_VFD[%d]: Setting up..."), slaveId);
    ...
    if (!block)
    {
        L_ERROR(F("SAKO_VFD[%d]: Failed to add mandatory read block (Addr=0x%04X, Count=%d)!"), slaveId, startAddr, SAKO_MB_MONITOR_REGS);
...
short SAKO_VFD::info()
{
    ...
    L_INFO(F("--- SAKO_VFD[%d] Info ---"), slaveId);
    ...
    L_INFO(F("--- End SAKO_VFD[%d] Info --- "), slaveId);
    return 0;
}

bool SAKO_VFD::onRegisterUpdate(uint16_t address, uint16_t newValue)
{
    lastErrorCode = 0;
    bool updated = false;
...
bool SAKO_VFD::setFrequency(uint16_t value)
{
    ...
    L_INFO(F("SAKO_VFD[%d]: Setting Frequency Output (Addr=%d, Val=%d Hz"), slaveId, SAKO_REG_SET_FREQ, value);    
...
bool SAKO_VFD::run() // Assuming run means forward
{
    ...
    // L_INFO(F("SAKO_VFD[%d]: Setting RUN command (Addr=0x%04X, Val=0x%04X)"), slaveId, reg, cmd);
    L_INFO(F("SAKO_VFD[%d]: Setting RUN command (Addr=%d, Val=%d)"), slaveId, reg, cmd);
...
bool SAKO_VFD::reverse()
{
    ...
    uint16_t reg = static_cast<uint16_t>(E_SAKO_REGISTERS::E_SAKO_REGISTERS_SET_DIR);
    // L_INFO(F("SAKO_VFD[%d]: Setting REVERSE command (Addr=0x%04X, Val=0x%04X)"), slaveId, reg, cmd);
    L_INFO(F("SAKO_VFD[%d]: Setting Reverse command (Addr=%d, Val=%d)"), slaveId, reg, cmd);
    this->setOutputRegisterValue(reg, cmd);
    return true;
}

short SAKO_VFD::stop() // Using Deceleration Stop
{
...
bool SAKO_VFD::resetFault()
{
    ...
    uint16_t reg = static_cast<uint16_t>(E_SAKO_REGISTERS::E_SAKO_REGISTERS_SET_DIR);
    L_INFO(F("SAKO_VFD[%d]: Setting RESET command (Addr=0x%04X, Val=0x%04X)"), slaveId, reg, cmd);
    this->setOutputRegisterValue(reg, cmd);
    return true;
}
...
bool SAKO_VFD::retract()
{
    ...
    L_INFO(F("SAKO_VFD[%d]: Starting retract sequence..."), slaveId);
    ...
    {
        L_ERROR(F("SAKO_VFD[%d]: Failed to send initial stop command for retract."), slaveId);
        return false;
    }
}
...
short SAKO_VFD::mb_tcp_read(MB_Registers *reg)
{
    if (!reg)
    {
        L_ERROR(F("SAKO_VFD[%d]::mb_tcp_read - Invalid MB_Registers pointer"), this->slaveId);
    ...
    { // Handle cases where TCP mapping might not be configured
        L_ERROR(F("SAKO_VFD[%d]::mb_tcp_read - TCP Base Address is 0"), this->slaveId);
        return (short)MB_Error::ServerDeviceFailure;
    }
...
short SAKO_VFD::mb_tcp_write(MB_Registers *reg, short value)
{
    ...
    if (tcpBaseAddr == 0)
    {
        L_ERROR(F("SAKO_VFD[%d]::mb_tcp_write - TCP Base Address is 0"), this->slaveId);
        return (short)MB_Error::ServerDeviceFailure;
    }
    ...
    case E_SakoTcpOffset::CMD_DIRECTION:
    {
        ...
        case E_SAKO_DIRECTION::E_SAKO_DIR_FREE_STOP:
            ...
            L_INFO(F("SAKO_VFD[%d]: Setting FREE STOP command (Addr=0x%04X, Val=0x%04X)"), slaveId, regAddr, value);
    ...
    case E_SakoTcpOffset::TARGET_REGISTER:
        ...
        L_INFO(F("SAKO_VFD[%d]: TCP Target Register set to 0x%04X"), this->slaveId, _tcpTargetRegister);
        commandSuccess = true;
        break;
    case E_SakoTcpOffset::TARGET_VALUE:
        ...
        else
        {
            ...
            if (rs485)
            {
                L_INFO(F("SAKO_VFD[%d]: TCP Writing Value 0x%04X to VFD Register 0x%04X"), this->slaveId, value, _tcpTargetRegister);
                ...
                if (commandSuccess)
                {
                    L_INFO(F("SAKO_VFD[%d]: VFD Register 0x%04X write successful."), this->slaveId, _tcpTargetRegister);
                }
                else
                {
                    L_ERROR(F("SAKO_VFD[%d]: VFD Register 0x%04X write failed. Status: %d"), this->slaveId, _tcpTargetRegister, static_cast<int>(writeStatus));
                }
            ...
            {
                L_ERROR(F("SAKO_VFD[%d]: RS485 owner not found for VFD write."), this->slaveId);
                commandSuccess = false;
...
void SAKO_VFD::_updateStatusFromRegister(uint16_t statusReg)
{
    ...
    //     L_INFO(F("SAKO_VFD[%d]: Running state changed to %s"), slaveId, currentRunning ? "Running" : "Stopped");
    // }
...
void SAKO_VFD::_updateVfdState()
{
    ...
    {
        L_INFO(F("SAKO_VFD[%d]: VFD State changed from %d to %d"), slaveId, previousState, _vfdState);
...
void SAKO_VFD::onError(ushort errorCode, const char* errorMessage)
{
    RTU_Base::onError(errorCode, errorMessage);
}

ushort SAKO_VFD::mb_tcp_error(MB_Registers *reg)
{
