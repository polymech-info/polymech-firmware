...
cp ./clients/mesint/slave/* ./dist/data/
osr-sync zip --source=./dist/ --target=./clients/mesint/slave.zip --cwd=./dist --profile=./sync-clients.json --verbose --debug

sh scripts/deploy-mesint.sh
