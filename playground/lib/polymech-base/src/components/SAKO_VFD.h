...
enum class E_SakoTcpOffset : ushort
{
    ...
    CMD_FREQ = 9,              // Write Frequency (Uses SAKO_REG_SET_FREQ)
    CMD_DIRECTION = 10,        // Write Direction/Control (Uses E_SAKO_REGISTERS_SET_DIR)
    CMD_COMMAND = 11,          // Internal command
    TARGET_REGISTER = 12,      // Write target register address
    TARGET_VALUE = 13,         // Write value to target register
};

class SAKO_VFD : public RTU_Base
{
public:
    static constexpr int SAKO_TCP_BLOCK_COUNT = 13;
    // Constructor
    SAKO_VFD(uint8_t slaveId, millis_t readInterval = SAKO_VFD_DEFAULT_READ_INTERVAL);
