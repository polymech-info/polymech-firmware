...
short ModbusTCP::setup()
{
    ...
    auto readCoilsWorker = [this](ModbusMessage request) -> ModbusMessage
    {
        ...
        for (uint16_t i = 0; i < count; ++i)
        {
            ...
            {
                Log.verboseln("ModbusTCP: Read Coil FC=01 Addr=%d - No mapping found.", currentAddr);
                anyError = false; // Don't flag missing mapping as a hard error for the response
            }
    ...
    auto writeSingleCoilWorker = [this](ModbusMessage request) -> ModbusMessage
    {
        ...
        if (!reg)
        {
            Log.warningln("ModbusTCP: Write Coil FC=05 Addr=%d - No mapping found.", address);
            response.setError(request.getServerID(), request.getFunctionCode(), Modbus::ILLEGAL_DATA_ADDRESS);
            return response;
        }
    ...
    auto writeSingleHregWorker = [this](ModbusMessage request) -> ModbusMessage
    {
        ...
        if (!reg)
        {
            Log.warningln("ModbusTCP: Write HReg FC=06 Addr=%d - No mapping found.", address);
            response.setError(request.getServerID(), request.getFunctionCode(), Modbus::ILLEGAL_DATA_ADDRESS);
            return response;
        }
        ...
        {
            Log.warningln("ModbusTCP: Write HReg FC=06 Addr=%d - Invalid type (%d) or access (%d) in mapping.", address, reg->type, reg->access);
            response.setError(request.getServerID(), request.getFunctionCode(), Modbus::ILLEGAL_DATA_ADDRESS);
            return response;
        }
    ...
    auto writeMultiCoilsWorker = [this](ModbusMessage request) -> ModbusMessage
    {
        ...
        for (uint16_t i = 0; i < count; ++i)
        {
            ...
            if (reg)
            {
                ...
                {
                    Log.warningln("ModbusTCP: Write Multi Coil FC=15 Addr=%d - Invalid type (%d) or access (%d).", currentAddr, reg->type, reg->access);
                }
            }
            else
            {
                Log.verboseln("ModbusTCP: Write Multi Coil FC=15 Addr=%d - No mapping found.", currentAddr);
            }
    ...
    auto writeMultiHregsWorker = [this](ModbusMessage request) -> ModbusMessage
    {
        ...
        for (uint16_t i = 0; i < count; ++i)
        {
            ...
            if (reg)
            {
                ...
                {
                    Log.warningln("ModbusTCP: Write Multi HReg FC=16 Addr=%d - Invalid type (%d) or access (%d).", currentAddr, reg->type, reg->access);
                }
            }
            else
            {
                Log.verboseln("ModbusTCP: Write Multi HReg FC=16 Addr=%d - No mapping found.", currentAddr);
            }
        }
...
bool ModbusTCP::registerModbus(Component *component, const MB_Registers &info)
{
    ...
    {
        Log.warningln("ModbusTCP::registerModbus - Component ID %d (%s) provided invalid Modbus info block (Address: %d, Count: %d). Skipping block.",
                      component->id, component->name.c_str(), info.startAddress, info.count);
        return false;
    }

    if (addressMappings.size() >= MAX_TCP_MAPPINGS)
    {
        Log.errorln("ModbusTCP::registerModbus - Max Modbus components/mappings reached (%d). Cannot register block for Component ID %d (%s).",
                    MAX_TCP_MAPPINGS, component->id, component->name.c_str());
        return false;
    }
