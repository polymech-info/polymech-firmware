...
import { getSlaveIdFromGroup, ControllerConfig } from '@/lib/controllerUtils';
import { T } from '../i18n';
...
const ControllerChart: React.FC = () => {
  const { registers: allModbusRegisters, isConnected, profiles, settings } = useModbus();
  ...
  const partitionedControllers = useMemo((): PartitionDisplayInfo[] => {
    ...
    settings.partitions.forEach(partition => {
      ...
      if (!partition.controllers && partition.startslaveid !== undefined && partition.numcontrollers !== undefined) {
        currentPCs = [];
        for (let i = 0; i < partition.numcontrollers; i++) currentPCs.push({ slaveid: partition.startslaveid + i, name: `Ctrl ${partition.startslaveid + i}`, enabled: true });
      }
  ...
  const activeProfile = useMemo(() => profiles?.find(p => p.status === PlotStatus.RUNNING || p.status === PlotStatus.PAUSED || p.status === PlotStatus.WAITING), [profiles]);
  ...
  return (
    <div id="controller-chart" className="md:bg-card p-2 md:p-4 mt-2 rounded-lg shadow-lg space-y-4">
