...
import ProfileOverrides, { OffsetData } from './ProfileOverrides';
...
const samplePressureControlPoints: ControlPoint[] = [
    { x: 0, y: 0.7 },
    { x: 0.59, y: 1.0 },
    { x: 1.0, y: 1.0 },
    { x: 1.0, y: 0.0 }
...
const ProfileEditor: React.FC<ProfileEditorProps> = ({
    ...
    const [children, setChildren] = useState<number[]>((initialData?.children || []).filter(id => id !== 0));
    ...
    const [overrides, setOverrides] = useState<OffsetData[]>(initialData?.overrides?.sp || []);
    ...
    const isRunning = liveProfileData?.status === PlotStatus.RUNNING || liveProfileData?.status === PlotStatus.PAUSED;
    const elapsedTime = liveProfileData?.elapsed;

    // --- START: Signal Plot State ---
    const { registers: allModbusRegisters, coils: allModbusCoils, pressureProfiles, savePressureProfile } = useModbus();
    ...
    // --- START: Pressure Profile State ---
    const [pressureProfileId, setPressureProfileId] = useState<number>(() => {
        const value = initialData?.pressureProfile ?? -1;
        return value;
    });
    const [selectedPressureProfile, setSelectedPressureProfile] = useState<any | null>(null);
    const [pressureControlPoints, setPressureControlPoints] = useState<ControlPoint[]>([]);
    const [activeView, setActiveView] = useState<'temperature' | 'pressure'>('temperature');
    const [isPressureProfileDirty, setIsPressureProfileDirty] = useState(false);
    const [pressureTargetRegisters, setPressureTargetRegisters] = useState<number[]>([]);
        ...
        const fetchAllProfiles = async () => {
            try {
                ...
                const filteredProfiles = initialData
                    ? profiles.filter(p => p.id !== initialData.id)
                    : profiles;
    ...
    // Effect to update pressureProfileId when initialData changes
    useEffect(() => {
        if (initialData?.pressureProfile !== undefined) {
            setPressureProfileId(initialData.pressureProfile);
        }
    }, [initialData?.pressureProfile]);

    // Effect to load selected pressure profile
    useEffect(() => {
        if (pressureProfileId >= 0 && pressureProfiles.length > 0) {
            const profile = pressureProfiles.find(p => p.slot === pressureProfileId);
            setSelectedPressureProfile(profile || null);

            // Convert pressure control points to UI format
            if (profile && profile.controlPoints) {
                const uiControlPoints = profile.controlPoints.map((cp: any) => ({
                    x: cp.x / 1000, // Convert from 0-1000 to 0-1
                    y: cp.y / 1000  // Convert from 0-1000 to 0-1
                }));
                setPressureControlPoints(uiControlPoints);
            } else {
                setPressureControlPoints([]);
            }

            // Load target registers for pressure profile
            if (profile && profile.targetRegisters) {
                const targetRegs = Array.isArray(profile.targetRegisters)
                    ? profile.targetRegisters.filter((reg: any) => typeof reg === 'number' && reg > 0)
                    : [];
                setPressureTargetRegisters(targetRegs);
            } else {
                setPressureTargetRegisters([]);
            }
        } else {
            setSelectedPressureProfile(null);
            setPressureControlPoints([]);
            setPressureTargetRegisters([]);
        }
    }, [pressureProfileId, pressureProfiles]);

    // --- START: Pressure Profile Effects and Handlers ---
    const handlePressureProfileSave = async () => {
        console.log('DEBUG - handlePressureProfileSave');
        if (!selectedPressureProfile) return;
        try {
            // Convert UI control points back to service format (0-1 to 0-1000)
            const serviceControlPoints = pressureControlPoints.map(cp => ({
                x: Math.round(cp.x * 1000),
                y: Math.round(cp.y * 1000)
            }));

            // Update the pressure profile with new control points and target registers
            const updatedPressureProfile = {
                ...selectedPressureProfile,
                controlPoints: serviceControlPoints,
                duration: duration, // Sync duration with temperature profile
                targetRegisters: pressureTargetRegisters
            };

            await savePressureProfile(updatedPressureProfile);

            toast({
                title: translate("Pressure Profile Saved"),
                description: translate('Profile "{name}" was saved successfully.').replace('{name}', selectedPressureProfile.name)
            });
            setIsPressureProfileDirty(false);
        } catch (error) {
            toast({
                title: translate("Error"),
                description: translate('Failed to save pressure profile "{name}".').replace('{name}', selectedPressureProfile.name),
                variant: "destructive"
            });
        }
    };

    // Wrapper function to mark pressure profile as dirty when control points change
    const handlePressureControlPointsChange = (newControlPoints: ControlPoint[]) => {
        setPressureControlPoints(newControlPoints);
        setIsPressureProfileDirty(true);
    };

    const handleSamplePressureControlPoints = () => {
        setPressureControlPoints(samplePressureControlPoints);
        setIsPressureProfileDirty(true);
    };
    ...
    useEffect(() => {
        const fetchPlot = async (plotId: number) => {
            try {
                const plots = await modbusApiService.getSignalPlots();
                const plot = plots.find((p: SignalPlotData) => p.slot === plotId);
    ...
    const handleExportMarkdown = () => {
        ...
        formData.controlPoints.forEach((p) => {
    ...
    const handleExportJson = () => {
        ...
        const profileToExport = {
            ...
            signalPlot: signalPlotId,
            children,
            enabled: initialData?.enabled ? true : false,
            pressureProfile: pressureProfileId !== -1 ? pressureProfileId : undefined,
        ...
        reader.onload = (e) => {
            try {
                ...
                if (Array.isArray(importedProfile.controlPoints)) {
                    ...
                    const uiControlPoints = importedProfile.controlPoints.map((p: ControlPoint) => ({
                ...
                if (typeof importedProfile.pressureProfile === 'number') {
                    setPressureProfileId(importedProfile.pressureProfile);
                }
                if (Array.isArray(importedProfile.children)) {
                    setChildren(importedProfile.children.filter((c: any) => typeof c === 'number'));
                }
                if (Array.isArray(importedProfile.pressureTargetRegisters)) {
                    setPressureTargetRegisters(importedProfile.pressureTargetRegisters.filter((tr: any) => typeof tr === 'number'));
                }
    ...
    const handleSubmit = () => {
        const submitData: ProfileSavePayload = {
            ...
            pressureProfile: pressureProfileId !== -1 ? pressureProfileId : undefined,
            children: children,
            overrides: overrides.length > 0 ? { sp: overrides } : undefined,
        };
        onSubmit(submitData);
    }
    ...
    const profileOptions: Option[] = availableProfiles.filter(p => p.id !== 0).map((p: TemperatureProfile) => ({
        label: `${p.name} (Slot: ${p.slot}, ID: ${p.id})`,
        value: p.id.toString(),
    }));
    ...
    return (
        <div className="space-y-4 glass-panel p-4 rounded-xl">
            ...
            <div>
                <Label className="text-slate-700 dark:text-white"><T>Duration (hh:mm:ss)</T></Label>
                <TimeCodeEditor
            ...
            <div className="space-y-2 border-t border-slate-300/30 dark:border-white/10 pt-4">
                <label className="text-base font-medium glass-text">
                    <T>Profile Curves</T>
                </label>
                <div className='pt-2'>
                    <div className="grid grid-cols-1 gap-4">
                        <div>
                            {/* Profile Type Toggles - Show when pressure profile is selected */}
                            {selectedPressureProfile && (
                                <div className="space-y-2">
                                    <div className="flex flex-wrap justify-center gap-2 p-2 glass-card rounded-md">
                                        <span className="text-sm font-medium text-slate-700 dark:text-white mr-4 self-center">
                                            <T>Editing Mode:</T>
                                        </span>
                                        <Button
                                            type="button"
                                            ...
                                            className={`glass-button ${activeView === 'temperature'
                                                ? 'status-gradient-error text-white'
                                                : 'opacity-50 text-slate-500 dark:text-slate-400'
                                                } border-0`}
                                        >
                                            🌡️ <T>Temperature</T>
                                        </Button>
                                        <Button
                                            ...
                                            className={`glass-button ${activeView === 'pressure'
                                                ? 'status-gradient-connected text-white'
                                                : 'opacity-50 text-slate-500 dark:text-slate-400'
                                                } border-0`}
                                        >
                                            💨 <T>Pressure</T>
                                        ...
                                        <Button
                                            size="sm"
                                            type="button"
                                            onClick={handlePressureProfileSave}
                                            disabled={!isPressureProfileDirty}
                                            className="status-gradient-connected text-white border-0 font-medium"
                                        >
                                            <Save className="h-4 w-4 mr-2" />
                                            <T>Save Pressure Profile</T>
                                        </Button>
                                        {activeView === 'pressure' && (
                                            <Button
                                                size="sm"
                                                type="button"
                                                onClick={handleSamplePressureControlPoints}
                                                className="glass-button status-gradient-warning text-white border-0 font-medium"
                                            >
                                                <T>Sample Curve</T>
                                            </Button>
                                        )}
                        ...
                        <div className="w-full">
                            <BezierEditor
                                controlPoints={activeView === 'pressure' ? pressureControlPoints : controlPoints}
                                onChange={activeView === 'pressure' ? handlePressureControlPointsChange : setControlPoints}
                                max={activeView === 'pressure' ? (selectedPressureProfile?.max || 100) : tempRange.max}
                                duration={duration}
                                onTempRangeChange={activeView === 'pressure' ? undefined : handleTempRangeChange}
                                className="border rounded-md"
                                isRunning={isRunning}
                                elapsedTime={elapsedTime}
                                signalControlPoints={signalPlotData?.controlPoints}
                                pressureProfile={selectedPressureProfile}
                                activeView={activeView}
                                temperatureControlPoints={controlPoints}
                            />
                        </div>
                    </div>
                </div>
                ...
                >
                    <div >
                        ...
                        <Button
                            ...
                            className="mt-2 glass-button status-gradient-error text-white border-0"
                        >
                ...
                {signalPlotData && (
                    <CollapsibleSection
                        ...
                        buttonClassName="text-slate-600 dark:text-white/80 hover:text-slate-800 dark:hover:text-white"
                        headerContent={
                            <Button size="sm" type="button" onClick={(e) => { e.stopPropagation(); handleSignalPlotSave(); }} disabled={!isSignalPlotDirty} className="glass-button status-gradient-connected text-white border-0">
                                <Save className="h-4 w-4 mr-2" />
                    ...
                    >
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-slate-300/30 dark:border-white/10">
                            <div className="space-y-2">
                                <h3 className="text-sm font-medium text-slate-600 dark:text-slate-300"><T>Control Points</T> ({signalPlotData.controlPoints.length}):</h3>
                            ...
                            <div className="space-y-2">
                                ...
                                {selectedSignalCpInfo ? (
                                    ...
                                    />
                                ) : (<p className="text-xs text-slate-500 dark:text-slate-400 italic">{signalPlotData.controlPoints.length > 0 ? <T>Select a control point to see its properties.</T> : <T>No control points to select.</T>}</p>)}
                            </div>
                        </div>
                    </CollapsibleSection>
                )}
                ...
                <div className="border-t border-slate-300/30 dark:border-white/10 pt-4">
                    <Label className="text-slate-700 dark:text-white"><T>Associated Signal Plot (Optional)</T></Label>
                    <Select
                        onValueChange={(value) => {
                    ...
                    >
                        ...
                        <SelectContent className="glass-panel border-0">
                            <SelectItem value="@none" className="text-slate-700 dark:text-white hover:bg-slate-100 dark:hover:bg-white/10"><T>None</T></SelectItem>
                            {availableSignalPlots.map((plot, index) => (
                                <SelectItem key={`${plot.slot}_${index}`} value={plot.slot.toString()} className="text-slate-700 dark:text-white hover:bg-slate-100 dark:hover:bg-white/10">
                ...
                {ENABLE_TEMPERATURE_PRESSURE_PROFILE && (
                    <div className="border-t border-slate-300/30 dark:border-white/10 pt-4">
                        <Label className="text-slate-700 dark:text-white"><T>Associated Pressure Profile (Optional)</T></Label>
                        <Select
                            onValueChange={(value) => {
                                const newId = value === "@none" ? -1 : parseInt(value, 10);
                                setPressureProfileId(newId);
                            }}
                            value={pressureProfileId === -1 ? "@none" : pressureProfileId.toString()}
                        >
                            <SelectTrigger className="glass-input">
                            ...
                            <SelectContent className="glass-panel border-0">
                                ...
                                {pressureProfiles.map((profile, index) => (
                                    <SelectItem key={`${profile.id}_${index}`} value={profile.slot.toString()} className="text-slate-700 dark:text-white hover:bg-slate-100 dark:hover:bg-white/10">
                                        {profile.name} ({translate("Slot:")} {profile.slot}, {translate("Max:")} {profile.max})
                                    </SelectItem>
                                ))}
                            </SelectContent>
                        </Select>
                        ...
                        {selectedPressureProfile && (
                            <div className="mt-4 p-3 glass-card rounded-md">
                                <div className="text-sm text-slate-700 dark:text-white">
                                    <T>Selected:</T> <strong>{selectedPressureProfile.name}</strong>
                                </div>
                                <div className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                                    <T>Duration will be synchronized with temperature profile</T>
                                </div>
                            </div>
                        )}
                        ...
                        {selectedPressureProfile && (
                            <CollapsibleSection
                                id="pressure-profile-targets"
                                title={<T>Pressure Profile Target Registers</T>}
                                storageKey="pressure-profile-targets-collapsible"
                            ...
                            >
                                <div className="space-y-3">
                                    <div className="text-xs text-slate-500 dark:text-slate-400">
                                        <T>Select registers that this pressure profile will control</T>
                                    </div>

                                    <div className="space-y-2">
                                        {pressureTargetRegisters.map((address, index) => (
                                            <div key={index} className="flex items-center gap-2 p-2 glass-card rounded">
                                                <span className="text-sm font-medium text-slate-700 dark:text-white min-w-0 flex-1">
                                                    <T>Register</T> {address}
                                                </span>
                                                <Button
                                                    type="button"
                                                    size="sm"
                                                    onClick={() => {
                                                        const newTargets = pressureTargetRegisters.filter((_, i) => i !== index);
                                                        setPressureTargetRegisters(newTargets);
                                                        setIsPressureProfileDirty(true);
                                                    }}
                                                    className="glass-button status-gradient-error text-white border-0 text-xs px-2 py-1"
                                                >
                                                    <T>Remove</T>
                                                </Button>
                                            </div>
                                        ...
                                        {pressureTargetRegisters.length === 0 && (
                                            <div className="text-sm text-slate-500 dark:text-slate-400 italic text-center py-4">
                                                <T>No target registers selected</T>
                                            </div>
                                        )}
                                    </div>

                                    <div className="border-t border-slate-300/30 dark:border-white/10 pt-3">
                                        <AddressPickerWidget
                                            isEditMode={false}
                                            slaveId={0}
                                            onPropsChange={(props) => {
                                                if (props.selectedAddress && !pressureTargetRegisters.includes(props.selectedAddress)) {
                                                    setPressureTargetRegisters([...pressureTargetRegisters, props.selectedAddress]);
                                                    setIsPressureProfileDirty(true);
                                                    toast({
                                                        title: translate("Register Added"),
                                                        description: translate("Register {address} added to pressure profile").replace('{address}', props.selectedAddress.toString())
                                                    });
                                                }
                                            }}
                                        />
                ...
                <div className="border-t border-slate-300/30 dark:border-white/10 pt-4">
                    <Label className="text-slate-700 dark:text-white"><T>Child Profiles (Sub-plots)</T></Label>
                    <MultipleSelector
                        value={children.map(c => c.toString())}
                        onValueChange={(value) => {
                            setChildren(value.map(v => parseInt(v, 10)));
                        }}
                        defaultValue={children.map(c => c.toString())}
                ...
                <div className="space-y-2 border-t border-slate-300/30 dark:border-white/10 pt-4">
                    <div className="flex justify-between items-center mb-2">
                        <Label className="text-slate-700 dark:text-white"><T>Target Controllers (Registers)</T></Label>
                        <div className="flex gap-2">
                            <Button type="button" size="sm" onClick={handleAddAll} className="glass-button status-gradient-connected text-white border-0">
                                <T>Add all</T>
                            </Button>
                            <Button type="button" size="sm" onClick={handleRemoveAll} className="glass-button status-gradient-error text-white border-0">
                                <T>Remove all</T>
                            </Button>
                        </div>
                    </div>
                    <div className="grid grid-cols-2 gap-2 p-3 glass-card rounded-md max-h-32 overflow-y-auto">
                                ...
                                return (
                                    <div key={controller.id} className="flex items-center space-x-2">
                                        <input
                                            ...
                                            className="h-4 w-4 accent-emerald-500 transition duration-150 ease-in-out"
                                        />
                                        <label
                                            htmlFor={`controller-sp-cmd-${address}`}
                                            className="text-sm font-medium text-slate-700 dark:text-white cursor-pointer"
                                        >
                        ...
                        {controllersWithSpAddress.length === 0 && (
                            <p className="text-sm text-slate-500 dark:text-slate-400 col-span-2"><T>No controllers available.</T></p>
                        )}
                    </div>
                </div>
                ...
                <div className="border-t border-slate-300/30 dark:border-white/10 pt-4">
                    <ProfileOverrides
                        selectedTargetRegisters={selectedTargetRegisters}
            ...
            <div className="space-y-3 border-t border-slate-300/30 dark:border-white/10 pt-4 mt-6">
                <div className="grid grid-cols-3 gap-2">
                    <Button type="button" onClick={handleExportJson} className="glass-button">
                        <Download className="mr-2 h-4 w-4" />
                        <T>Export</T>
                    </Button>
                    <Button type="button" onClick={handleExportMarkdown} className="glass-button">
                        <Download className="mr-2 h-4 w-4" />
                        <T>Markdown</T>
                    </Button>
                    <Button type="button" onClick={triggerFileSelect} className="glass-button bg-gradient-to-r from-cyan-400 to-blue-500 text-white border-0">
                ...
                <input
                    ...
                    style={{ display: 'none' }}
                />
                <Button onClick={handleSubmit} className="w-full status-gradient-connected text-white border-0 text-lg font-semibold py-3">
            ...
            <AlertDialog open={showDeleteConfirmDialog} onOpenChange={setShowDeleteConfirmDialog}>
                <AlertDialogContent className="glass-panel border-0">
                    ...
                    <AlertDialogFooter>
                        <AlertDialogCancel onClick={() => setCpToDelete(null)} className="glass-button"><T>Cancel</T></AlertDialogCancel>
