...
import { BezierEditorProps } from './types';
import { toNormalizedCoords, toPathCoords } from './bezierUtils';
import BezierGrid from './BezierGrid';
import ControlPoints from './ControlPoints';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';

const BezierEditor: React.FC<BezierEditorProps & { yAxisLabel?: string, hideMaxInput?: boolean, unit?: string }> = ({
  controlPoints,
  onChange,
  max,
  className,
  ...
  // Generate the main curve path data (active editing curve)
  ...
  // Generate background pressure curve path data (when editing temperature)
  let pressurePathData = '';
  if (pressureProfile && pressureProfile.controlPoints && activeView === 'temperature') {
    const normalizedPressurePoints = pressureProfile.controlPoints.map((cp: any) => ({
      x: cp.x / 1000, // Convert from 0-1000 scale to 0-1
      y: cp.y / 1000  // Convert from 0-1000 scale to 0-1
    }));

    const pressurePathPoints = getBezierPath(normalizedPressurePoints);
    if (pressurePathPoints.length > 0) {
      const pressureSvgPoints = pressurePathPoints.map(point => toPathCoords(point, svgDimensions));
      pressurePathData = `M ${pressureSvgPoints[0].x},${pressureSvgPoints[0].y}`;

      for (let i = 1; i < pressureSvgPoints.length; i++) {
        pressurePathData += ` L ${pressureSvgPoints[i].x},${pressureSvgPoints[i].y}`;
      }
    }
  }
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    let { x, y } = point;
    const step = 0.05; // 5% change

    let keyHandled = false;
    switch (e.key) {
      case 'ArrowLeft':
        if (selectedPointIndex > 0 && selectedPointIndex < controlPoints.length - 1) {
          x -= step;
          keyHandled = true;
        }
        break;
      ...
      case 'ArrowDown':
        ...
        break;
      default:
        return; // Exit if it's not an arrow key we handle
    }
    ...
    // Clamp X if it was changed
    if (e.key === 'ArrowLeft' || e.key === 'ArrowRight') {
      const prevPoint = controlPoints[selectedPointIndex - 1];
      const nextPoint = controlPoints[selectedPointIndex + 1];
      x = Math.max(prevPoint.x, Math.min(nextPoint.x, x));
    }
    ...
    const newPoints = [...controlPoints];
    newPoints[selectedPointIndex] = { ...point, x: finalX, y: finalY };
    onChange(newPoints);
  };

  return (
    <div ref={editorRef} className="space-y-2 select-none" onKeyDown={handleKeyDown} tabIndex={0}>
      {!readonly && !hideMaxInput && (
        <div className="flex flex-row gap-4 mb-2">
          <div className="w-1/2">
            <Label htmlFor="max-temp">{yAxisLabel || "Max Temperature (°C)"}</Label>
            <Input
              id="max-temp"
              type="number"
              value={tempRange.max}
      ...
      >
        ...
        {/* Main curve - shows active editing mode */}
        <path
          d={pathData}
          stroke={activeView === 'pressure' ? "#2563eb" : "#dc2626"}
          strokeWidth="3"
          fill="none"
        />

        {/* Background curve - shows the other profile for reference */}
        {pressureProfile && pressurePathData && activeView === 'temperature' && (
          <path
            d={pressurePathData}
            className="pressure-curve-path"
            stroke="#2563eb"
            strokeWidth="2"
            fill="none"
            opacity="0.7"
        ...
        {/* Background temperature curve when editing pressure */}
        {pressureProfile && activeView === 'pressure' && temperatureControlPoints && (
          <path
            d={(() => {
              const tempPathPoints = getBezierPath(temperatureControlPoints);
              if (tempPathPoints.length === 0) return '';
              const tempSvgPoints = tempPathPoints.map(point => toPathCoords(point, svgDimensions));
              let tempPathData = `M ${tempSvgPoints[0].x},${tempSvgPoints[0].y}`;
              for (let i = 1; i < tempSvgPoints.length; i++) {
                tempPathData += ` L ${tempSvgPoints[i].x},${tempSvgPoints[i].y}`;
              }
              return tempPathData;
            })()}
            className="temperature-curve-background"
            stroke="#dc2626"
            strokeWidth="2"
            fill="none"
            opacity="0.7"
        ...
        {/* Signal Control Point Markers */}
        {signalControlPoints && signalControlPoints.map((cp, index) => {
          if (!duration || duration <= 0) return null;
          const cpRatio = (cp.time / 1000); // cp.time is 0-1000
          const { x: cpX } = toPathCoords({ x: cpRatio, y: 0 }, svgDimensions);
          const { y: bottomY } = toPathCoords({ x: 0, y: 0 }, svgDimensions); // y=0 is bottom

          // Render a small diamond for each signal CP
          return (
            <path
              key={`signal-cp-${index}`}
              d={`M ${cpX} ${bottomY - 5} L ${cpX + 5} ${bottomY} L ${cpX} ${bottomY + 5} L ${cpX - 5} ${bottomY} Z`}
              fill="purple"
              stroke="white"
              strokeWidth="1"
            />
