...
interface LEDBarsProps {
  ...
  max?: number;  // The value that corresponds to 100%
  label?: string;
  segments?: number;
...
const LEDBars: React.FC<LEDBarsProps> = ({
  ...
  const normalizedPercentage = Math.max(0, Math.min(200, percentage));

  const getSegmentColor = (segmentIndex: number, isActive: boolean): string => {
    const segmentValue = (segmentIndex + 1) * (200 / segments);

    let baseColorClass = 'bg-slate-300 dark:bg-slate-700';
    if (segmentValue <= 80) baseColorClass = 'bg-green-500';
    else if (segmentValue <= 100) baseColorClass = 'bg-yellow-500';
  ...
  const activeThreshold = Math.ceil(normalizedPercentage / (200 / segments));
