...
const formSchema = z.object({
  ...
  duration: z.coerce.number().min(10000, { message: translate('Duration must be at least 10 seconds (10000 ms)') }),
      ...
      >
        ...
        <div className="space-y-2">
          ...
          <BezierEditor
            ...
            max={tempRange.max}
            duration={form.watch('duration') / (60 * 1000)} // Convert ms to minutes for BezierEditor
            ...
            {availableControllers
              .map(controller => {
                if (!registers || registers.length === 0) {
                  return { controller, spCmdRegisterAddress: null };
                }
              ...
              .filter(item => item.spCmdRegisterAddress !== null)
              .map(({ controller, spCmdRegisterAddress }) => {
                const address = spCmdRegisterAddress!; // Non-null assertion due to filter
