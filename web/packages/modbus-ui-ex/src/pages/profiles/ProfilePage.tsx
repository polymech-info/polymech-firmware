...
import { transformControllerConfigsToProfileFormFormat, transformServiceProfileToUI, transformUIControlPointsToService } from '../../lib/profile-transformers';
import { T } from '@/i18n';
...
const ProfilePage = () => {
  ...
  useEffect(() => {
    const transformed = transformControllerConfigsToProfileFormFormat(PARTITION_CONFIG);
  ...
  }, [isConnected, fetchServiceProfiles, toast, serviceProfiles]);
  ...
  useEffect(() => {
    if (isEditMode && serviceProfiles.length > 0 && allModbusRegisters && allModbusCoils) {
      const profileSlot = parseInt(slot, 10);
      const serviceProfile = serviceProfiles.find(p => p.slot === profileSlot);
      if (serviceProfile) {
        const uiProfile = transformServiceProfileToUI(serviceProfile, allModbusRegisters, allModbusCoils, PARTITION_CONFIG);
  ...
  }, [slot, isEditMode, serviceProfiles, allModbusRegisters, allModbusCoils, navigate, toast]);

  const handleSubmit = async (data: Omit<UIProfile, 'id' | 'createdAt' | 'updatedAt' | 'associatedControllerNames' | 'status' | 'elapsed' | 'remaining' | 'currentTemp' | 'enabled'>) => {
    if (!saveProfile) {
    ...
    const profilePayload: ProfileSavePayload = {
      ...
      max: data.max,
      controlPoints: transformUIControlPointsToService(data.controlPoints) as any,
